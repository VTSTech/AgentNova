...
> Generated: 04-02-2026 | Auditor: Super-Z | Commit: 4572092 (R04.4)
...
| **Tech Stack** | Python 3.9+, zero runtime dependencies (stdlib only: urllib, json, subprocess, dataclasses, threading, sqlite3, ast) |
...
| **Version** | 0.4.4 (R04.4) · Status: Alpha |
...
config.py           → Centralized config from env vars. Factory functions for dynamic defaults. ACP credentials.
...
### `agentnova/backends/llama_server.py` — llama-server / BitNet Backend (merged, R04.2+R04.4)
...
- **Config**: `LLAMA_SERVER_BASE_URL` (localhost:8080), `BITNET_BASE_URL` (localhost:8765).
...
### `agentnova/config.py` — Central Configuration (228 lines)
- **Purpose**: Single source of truth for all URLs, defaults, security settings, ACP credentials.
- **Key defaults**: `OLLAMA_BASE_URL` (localhost:11434), `BITNET_BASE_URL` (localhost:8765), `LLAMA_SERVER_BASE_URL` (localhost:8080), `DEFAULT_MODEL` (backend-dependent: qwen2.5:0.5b for Ollama, bitnet-b1.58-2b-4t for BitNet, default for llama-server), `MAX_STEPS` (10).
...
### `agentnova/cli.py` — Full CLI (1786 lines)
- **Commands**: run, chat, agent, models, tools, test, version, config, modelfile, skills, soul, sessions, update (alphabetically ordered).
...
- **R04.4 changes**: BitNet model discovery in `_build_agent()` and `cmd_test()` via `backend.list_models()` → `/props`. CLI commands reordered alphabetically.
...
11. Run `pytest` after any change
...
> This brief is approximately **~8,000 tokens** (estimated: 28,000 characters ÷ 3.5). Target maximum: 16,000 tokens. Headroom: ~8,000 tokens (~50% of budget remaining).
