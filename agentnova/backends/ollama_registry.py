...
def _detect_weight_quant(blob_path: Path) -> str:
    """Read GGUF header to detect weight quantization type.

    GGUF v3 header layout (little-endian):
      offset 0:  magic     (4 bytes, uint32) = 0x46475547
      offset 4:  version   (4 bytes, uint32) = 3
      offset 8:  n_tensors (8 bytes, uint64)
      offset 16: n_kv      (8 bytes, uint64)

    After the header, metadata key-value pairs follow. We look for the
    "general.quantization_version" and "general.file_type" keys which
    tell us the quantization. However, the simplest heuristic is to
    check tensor count vs file size, or read the first tensor type.

    For reliability, we scan for the "general.file_type" metadata entry
    which maps to GGML quant type constants used by llama.cpp/ggml.
    """
    try:
        with open(blob_path, "rb") as f:
            ...
            # GGUF string format: length (uint64) + data (bytes) + null terminator
            # We need to scan KV pairs to find "general.file_type"
            for _ in range(n_kv):
                # Read key (GGUF string)
                ...
                if val_type == 8:  # GGUF_TYPE_STRING
                    ...
                    f.read(val_len)  # skip value
                elif val_type == 4:  # GGUF_TYPE_UINT32
                    f.read(4)
                elif val_type == 6:  # GGUF_TYPE_INT32
                    f.read(4)
                elif val_type == 7:  # GGUF_TYPE_FLOAT32
                    f.read(4)
                elif val_type == 9:  # GGUF_TYPE_BOOLEAN
                    f.read(1)
                elif val_type == 10:  # GGUF_TYPE_ARRAY
                    ...
                    for _ in range(arr_len):
                        if arr_type == 8:  # STRING
                            ...
                            f.read(elem_len)
                        elif arr_type in (4, 6, 7):
                            f.read(4)
                        elif arr_type == 9:
                            f.read(1)
                        else:
                            break  # unknown element type, stop scanning
                else:
                    # Unknown type - can't continue scanning reliably
                    # Fall back to heuristic
                    break

                if key == "general.file_type" and val_type == 4:
                    # We already read the uint32 value above but didn't save it
                    # Need to re-read - seek back
                    break

                if key == "general.quantization_version":
                    # Continue scanning, file_type usually follows
                    pass

            # If we successfully scanned KV pairs, re-read file_type
            # Actually, we need a different approach - save the value as we go
            # Let's do it properly in a second pass

    except (OSError, struct.error, UnicodeDecodeError, OverflowError):
        return "UNKNOWN"

    # Fallback: heuristic from filename
    name_lower = blob_path.stem.lower()
    for qt in ("q8_0", "q4_k_m", "q4_k_s", "q4_0", "q5_k_m", "q5_0", "q5_k_s",
               "q6_k", "q2_k", "q3_k_m", "q3_k_s", "iq4_xs", "iq4_nl",
               "f16", "f32", "bf16", "q8_0", "tq4_1s", "tq3_1s"):
        if qt.replace("_", "").replace("-", "") in name_lower.replace("_", "").replace("-", ""):
            return qt.upper()

    return "UNKNOWN"


def _detect_weight_quant_v2(blob_path: Path) -> str:
    ...
    try:
        with open(blob_path, "rb") as f:
            ...
            for _ in range(n_kv):
                ...
                if file_type is not None and key == "general.file_type":
                    break
    ...
    for library_dir in sorted(OLLAMA_MANIFESTS_DIR.iterdir()):
        ...
        for repo_dir in sorted(library_dir.iterdir()):
            ...
            for tag_file in sorted(repo_dir.iterdir()):
                ...
                if blob_path.exists():
                    weight_quant = _detect_weight_quant_v2(blob_path)
                else:
