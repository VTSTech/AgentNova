...
def parse_compatibility(compatibility_str: str) -> Dict[str, Any]:
    ...
    for part in parts:
        ...
        version_match = re.match(r'^(\w+)\s*(>=|<=|>|<|==|!=)\s*([\d.]+)
...
, self.name):
    ...
    def _validate_license(self):
        """Validate the license field against SPDX identifiers."""
        if self.license:
    ...
    def _parse_compatibility(self):
        """Parse compatibility field into structured data."""
    ...
    def license_warning(self) -> str:
        """Get license validation warning message."""
    ...
    def check_compatibility(self, runtime: str = None, python_version: str = None) -> tuple[bool, List[str]]:
        """
        ...
        if not self._compatibility_parsed:
            return True, []
        
        # Check Python version
        if python_version and self._compatibility_parsed.get("python"):
            py_req = self._compatibility_parsed["python"]
            req_version = py_req.get("min_version")
            op = py_req.get("operator", ">=")
            if req_version:
                # Simple version comparison (assumes >=)
                if op == ">=" and python_version < req_version:
                    warnings.append(f"Python {req_version}+ required, found {python_version}")
        
        # Check runtime
        if runtime and self._compatibility_parsed.get("runtimes"):
            runtimes = [r["name"].lower() for r in self._compatibility_parsed["runtimes"]]
            if runtimes and runtime.lower() not in runtimes:
                warnings.append(f"Skill prefers runtimes: {', '.join(runtimes)}")
